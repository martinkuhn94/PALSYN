...
from PALSYN.models.lnn import build_lnn_model

from .base import BaseSynthesizer


class LNNSynthesizer(BaseSynthesizer):
    MODEL_TYPE = "LNN"
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a differential privacy-aware liquid neural network synthesizer."""
        ...
        self.tau_min = float(model_cfg.get("tau_min", tau_min))
        self.tau_max = float(model_cfg.get("tau_max", tau_max))
        self.connectivity = float(model_cfg.get("connectivity", connectivity))
    ...
    def _build_model_impl(self) -> tuple[tf.keras.Model, list[str]]:
        model, modified_columns = build_lnn_model(
